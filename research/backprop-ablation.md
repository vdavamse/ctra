...
- **Adaptive branching never adapts *on this landscape*: every expansion ran at `visit_count == 1`.** With 7 addable features and `max_depth 7` the frontier exhausts before any node is re-selected: `search()` expands the selected leaf as soon as `visit_count > 0`, an expanded node below `max_depth` has children and is not returned by `_select` as a leaf again, and `_simulate_deep` likewise expands a node right after its first evaluation. Instrumenting `MCTSSearch._expand` over every cell of this sweep (11 cells × 3 rules × 20 seeds, 40,328 expansions) found no expansion at any visit count other than 1, so the AB-MCTS formula `clamp(floor(log2(visit_count)) + 2, min_branch_factor, max_branch_factor)` always evaluated to `max(min_branch_factor, 2)` clamped to `max_branch_factor` — branch 2 in every cell here — and the sweep's `adaptive` axis is a branch-2 vs branch-N comparison and nothing more. That is why the `deep-wide` adaptive cell (branch ≤ 8) equals the `saturated` deep r40/b2 cell on every metric of every seed (60/60 rows identical), why its "efficiency" (143 vs 248 evaluations, 167 vs 650 nodes, AUC 0.8427 vs 0.8374) is the efficiency of a narrower tree — on this landscape, where every ADD is a step toward the plateau, narrow and deep is simply better — and why the shallow r10 adaptive cell differs from its fixed neighbour (branch 2 vs 3: depth 2.90 vs 2.00). The result is the geometry's, not the formula's. The one place the count can grow is a node at `max_depth`: `_select` loops `while not current.is_leaf and depth < max_depth`, so it stops there and returns the node whether or not it has children; `search()` expands it with no leaf or depth check and `_expand` never checks `node.children`, so the node is re-expanded on every visit and mints duplicate children one level past `max_depth`. Probe (same landscape, deep, adaptive 2/8, 40 rollouts, seeds 0-2; `(visit_count, children produced)` counted at every `_expand` call): `max_depth 7` gives `{(1, 1): 120, (1, 2): 189}`, no duplicate `suggestion_index` among siblings, deepest node at depth 7; `max_depth 3` gives `{(1, 2): 45, (2, 3): 24, (3, 3): 24, (4, 4): 24}` — the `log2` term does grow there — with 192 duplicate `suggestion_index` children, 288 nodes past `max_depth` and the deepest node at depth 4. In production (`max_depth 10`, an LLM proposer that always suggests) the factor varies at the `max_depth` boundary, and the defect there is the duplicate expansion past `max_depth`: a separate, pre-existing defect the follow-up should fix before adaptive branching is re-evaluated (section 9). Independent of the backprop rule.
...
- **Nodes at `max_depth` are re-expanded on every visit, minting duplicate children past `max_depth`** (section 5). `_select` returns the node it stops at whether or not it has children, `search()` expands it with no leaf or depth check, and `_expand` never checks `node.children`. With `max_depth 3` on this landscape (deep, adaptive 2/8, 40 rollouts, 3 seeds) the probe shows expansions at visit counts 1-4 (`{1: 45, 2: 24, 3: 24, 4: 24}`), 192 duplicate `suggestion_index` children, 288 nodes past `max_depth` and a node at depth 4; at `max_depth 7` every one of 309 expansions ran at `visit_count == 1` and no node passed the depth. Fix the re-expansion (skip or re-select a selected node that already has children, or stop `_select` at a leaf) before re-evaluating AB-MCTS adaptive branching: on this landscape the `visit_count == 1` result is the geometry's — the 7-feature pool exhausts at `max_depth 7` — and the boundary re-expansion is the only place the `log2(visit_count)` term currently grows, so a measurement of `adaptive_branching` today measures that defect, not the formula.
